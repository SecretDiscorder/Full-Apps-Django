...
def spotify_player(request):
    static_dir = settings.BASE_DIR / 'apps/static/'
...
def images(request, img_type):
    image_path = os.path.join(settings.BASE_DIR, 'apps/images/', f'{img_type}.png')
...
def chess_piece_image(request, piece_type):
    ...
    image_path = os.path.join(settings.BASE_DIR, 'apps/img/chesspieces/wikipedia', f'{piece_type}.png')
    ...
    try:
        ...
        # Plot the polynomial if operation is 'roots'
            if operation == 'roots':
                ...
                plt.grid(True)
                plt.savefig('/apps/static/polynomial_plot.png')
                plt.close()
    except Exception as e:
        pass
